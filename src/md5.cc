...
main() {
    ...
    for (i = 0; i < 7; ++i) {
        ...
        printf("MD5 (\"%s\") = ", test[i]);
        for (di = 0; di < 16; ++di) printf("%02x", digest[di]);
        printf("\n");
    }
    return 0;
}
...
#undef SET
    ...
    void md5_finish(md5_state_t* pms, md5_byte_t digest[16]) {
        ...
        for (i = 0; i < 8; ++i)
            data[i] =
                static_cast<md5_byte_t>(pms->count[i >> 2] >> ((i & 3) << 3));
        ...
        for (i = 0; i < 16; ++i)
            digest[i] =
                static_cast<md5_byte_t>(pms->abcd[i >> 2] >> ((i & 3) << 3));
    }

    std::string md5_hash_string(const md5_byte_t* data, int nbytes) {
        char digest[16];
        md5_state_t state;
    ...
    std::string md5_hash_string(const std::string& s) {
        return md5_hash_string(reinterpret_cast<const md5_byte_t*>(s.c_str()),
                               s.size());
    }
    ...
    std::string md5_hash_hex(const std::string& input) {
        ...
        for (size_t i = 0; i < hash.size(); i++) {
            hex.push_back(kHEXVAL[((hash[i] >> 4) & 0xF)]);
            hex.push_back(kHEXVAL[(hash[i]) & 0x0F]);
        }
