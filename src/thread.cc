...
#include <thread>
        ...
        void afterFork() {
            lux::base::CurrentThread::t_cachedTid = 0;
            lux::base::CurrentThread::t_threadName = "main";
        ...
        struct ThreadData {
            ...
            void runThread() {
                *tid_ = lux::base::CurrentThread::tid();
                tid_ = nullptr;
                latch_->countDown();
                latch_ = nullptr;

                lux::base::CurrentThread::t_threadName =
                    name_.empty() ? "LuxThread" : name_.c_str();
                ::prctl(PR_SET_NAME, lux::base::CurrentThread::t_threadName);

                try {
                    func_();
                    lux::base::CurrentThread::t_threadName = "finished";
                } catch (const lux::base::Exception& ex) {
                    lux::base::CurrentThread::t_threadName = "crashed";
